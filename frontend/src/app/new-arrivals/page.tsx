...
function NewArrivalsContent() {
  ...
  const params = useMemo(() => {
    const p: Record<string, string | number | boolean> = {
      page: currentPage, page_size: 20, ordering: sortBy, is_new_arrival: "true",
    };
    ...
    return p;
  }, [searchQuery, selectedCategories, selectedSizes, sortBy, currentPage]);

  const { data, isLoading } = useQuery({
    queryKey: ["new-arrivals-products", params],
  ...
  const updateURL = useCallback(() => {
    ...
    router.push(`/new-arrivals?${p.toString()}`, { scroll: false });
  }, [searchQuery, selectedCategories, selectedSizes, sortBy, currentPage, router]);

  useEffect(() => { updateURL(); }, [updateURL]);

  const clearAllFilters = () => {
    setSearchQuery(""); setSelectedCategories([]); setSelectedSizes([]); setCurrentPage(1);
    router.push("/new-arrivals", { scroll: false });
  };
  ...
  const hasActiveFilters = searchQuery || selectedCategories.length > 0 || selectedSizes.length > 0;
  ...
  const SectionHeader = ({ title, filterKey }: { title: string; filterKey: string }) => (
    <button onClick={() => toggleFilter(filterKey)}
      ...
      {title}
      {expandedFilters[filterKey] ? <ChevronUp className="h-4 w-4 text-brand-400" /> : <ChevronDown className="h-4 w-4 text-brand-400" />}
    </button>
  );
  ...
  return (
    <main className="min-h-screen bg-white">
      <div className="relative bg-gradient-to-r from-brand-900 via-brand-800 to-accent-900 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-12 md:py-16 text-center">
          <p className="text-accent-400 text-sm tracking-[0.3em] uppercase mb-3">Just Dropped</p>
          <h1 className="font-heading text-3xl md:text-5xl font-bold text-white mb-3">New Arrivals</h1>
          <p className="text-brand-300 max-w-md mx-auto">Be the first to discover our latest collections and trending styles.</p>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <input type="text" placeholder="Search new arrivals..." value={searchQuery}
          ...
          <div className="flex-1 min-w-0">
            ...
            {!isLoading && productList.length === 0 && (
              <div className="text-center py-20">
                ...
                <h3 className="font-heading text-xl font-semibold text-brand-900 mb-2">No new arrivals found</h3>
                <p className="text-brand-500 mb-6">Check back soon for the latest additions.</p>
...
export default function NewArrivalsPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-white"><div className="bg-gradient-to-r from-brand-900 to-accent-900 py-12"><div className="max-w-7xl mx-auto px-4 text-center"><div className="h-8 w-48 bg-white/20 rounded animate-pulse mx-auto" /></div></div></div>}>
      <NewArrivalsContent />
    </Suspense>
  );
}
