...
function FeaturedContent() {
  ...
  const params = useMemo(() => {
    const p: Record<string, string | number | boolean> = {
      page: currentPage, page_size: 20, ordering: sortBy, is_featured: "true",
    };
  ...
  const updateURL = useCallback(() => {
    ...
    router.push(`/featured?${p.toString()}`, { scroll: false });
  ...
  const clearAllFilters = () => {
    ...
    router.push("/featured", { scroll: false });
  };
  ...
  return (
    <main className="min-h-screen bg-white">
      <div className="relative bg-gradient-to-r from-accent-700 via-accent-600 to-brand-900 overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div className="absolute top-0 right-0 w-64 h-64 bg-white rounded-full blur-3xl" />
          <div className="absolute bottom-0 left-0 w-48 h-48 bg-white rounded-full blur-3xl" />
        </div>
        <div className="relative z-10 max-w-7xl mx-auto px-4 py-12 md:py-16 text-center">
          <p className="text-white/70 text-sm tracking-[0.3em] uppercase mb-3">Handpicked for You</p>
          <h1 className="font-heading text-3xl md:text-5xl font-bold text-white mb-3">Featured Products</h1>
          <p className="text-white/80 max-w-md mx-auto">Our curated selection of standout pieces loved by our community.</p>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 py-6">
        <div className="mb-6">
          <div className="relative max-w-md">
            ...
            <input type="text" placeholder="Search featured products..." value={searchQuery}
          ...
          <div className="flex-1 min-w-0">
            ...
            {!isLoading && productList.length === 0 && (
              <div className="text-center py-20">
                ...
                <h3 className="font-heading text-xl font-semibold text-brand-900 mb-2">No featured products found</h3>
                <p className="text-brand-500 mb-6">Try adjusting your filters.</p>
...
export default function FeaturedPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-white"><div className="bg-gradient-to-r from-accent-700 to-brand-900 py-12"><div className="max-w-7xl mx-auto px-4 text-center"><div className="h-8 w-48 bg-white/20 rounded animate-pulse mx-auto" /></div></div></div>}>
      <FeaturedContent />
    </Suspense>
  );
}
